...
METHODS_TEACHER_LEC = [  #Методы преподавания лекций
    'Лекция',
    'Лекция-беседа',
    'Лекция-дискуссия',
    'Лекция с запланированными ошибками',
    'Лекция-исследование',
    'Лекция с применение техники обратной связи',
...
METHODS_TEACHER_PRAKT_LABS = [  #Методы преподавания практических или оабораторных работ
    'Виртуальная лабораторная работа',
    'Лабораторная работа',
    'Решение задач',
    'Работа с метод.указаниями'
]
...
TYPE_CONTROL_RATING = [ #виды контрольных мероприятий в рейтинге
     ...
     'Всего за 1-ую аттестацию',
     'Всего за 2-ую аттестацию',
     'Всего за 3-ую аттестацию'
]
...
def getTotalstr(semestrs, list_foradd): #Формирование списка надписей количества часов или баллов за семестр
    ...
    if len(sem_ochka)>1:
        for id in sem_ochka:
            res_ochka.append("Итого за {0} семестр:".format(id))

    res_ochka.append("Итого:")
    ...
    if (len(sem_z)==len(sem_zu)) and (len(sem_z)>1 and len(sem_zu)>1):
        for i in range(0,len(sem_z)):
            res_z.append("Итого за {0}/{1} семестр:".format(sem_z[i],sem_zu[i]))
