from django.conf.urls import url
from django.contrib.auth.decorators import login_required as auth
from . import views

urlpatterns = [
    url(r'^accounts/profile/(?P<slug>\w+)$',views.UserProfileDetailView.as_view(), name='profile'),
    url(r'^accounts/edit_profile/(?P<slug>\w+)$',auth(views.UserProfileEditView.as_view()), name='edit_profile'),
    url(r'^accounts/password/change/auth_password_change_done$',views.User_password_changed),
    url(r'^about/',views.showAuthors, name='showAuthors'),
    url(r'^news/',views.showNews, name='showNews'),
    url(r'^systeminfo/', views.showSysInfo, name='showSystemInfo'),
    url(r'^umk/list/filter/$',views.showUmkList_filters, name='umkfilter'),
    url(r'^umk/list/filter/(?P<training_program>(\w+))/$',views.showUmkList_filters, name='umkfilter'),
    url(r'^umk/list/filter/(?P<training_program>(\w+))/(?P<discipl>[+\w]+)/$',views.showUmkList_filters, name='umkfilter'),
    url(r'^umk/list/remove/(?P<id>(\d+))/$',views.remove_umk, name='umkremove'),
    url(r'^umk/list/',views.showUmkList, name='showumklist'),
    url(r'^umk/next/(?P<id>(\d+))/(?P<direct_id>(\d+))/(?P<tr_program>(\w+))/(?P<year>(\d+))/$', views.create_umk, name='umk_create'),
    url(r'^umk/edit/(?P<id>(\d+))/core/$',views.DataForUmk_core, name='umk_edit_core'),
    url(r'^umk/edit/(?P<id>(\d+))/laborat/$',views.DataForUmk_addons, {'type': 'laborat'}, name='edit_lab'),
    url(r'^umk/edit/(?P<id>(\d+))/prakt/$',views.DataForUmk_addons, {'type': 'prakt'}, name='edit_prakt'),
    url(r'^umk/edit/(?P<id>(\d+))/$',views.actions, {'type': 'actlist'}, name='umk_edit_menu'),
    url(r'^umk/edit/(?P<id>(\d+))/rating/$',views.show_rating, name='showrating'),
    url(r'^umk/edit/(?P<id>(\d+))/literature/$',views.show_liter, name='showliter'),
    url(r'^umk/edit/(?P<id>(\d+))/literature/get/(?P<type>(\w+))/(?P<search>[+\w]+)/$',views.get_literature_from_url, name='getliter'),
    url(r'^umk/get/(?P<id>(\d+))/(?P<format>[a-z]{3,4})/$', views.get_document),
    url(r'^umk/get/(?P<id_list>[+\d]+)/(?P<format>[a-z]{3,4})/$', views.get_document_in_archive),
    url(r'^umk/copy/', views.showUmkCopy, name='umkcopy'),
    url(r'^umk/send2sign/(?P<id>[+\d+]+)/$', views.send2signature, name='send2signtr'),
    url(r'^umk/zaf_kav_sign_list/$', views.showUmkForSignature, name='umklistsign'),
    url(r'^umk/zaf_kav_sign_confirm/(?P<id>[+\d+]+)/$', views.send2confirm, name='umklistconfirm'),
    url(r'^umk/create/',views.choiseDiscipline, name='umkchoise'),
    url(r'^plan/upload/',views.uploadplan, name='planupload'),
    url(r'^plan/list/',views.showPlansAll, name='planlist'),
    url(r'^plan/remove/(?P<id>(\d+))/$',views.removePlan, name='planremove'),
    url(r'^competence/upload/',views.uploadcompetence, name='uploadcompetence'),
    url(r'^umk/kos/(?P<id>(\d+))/$',views.kos_menu, name='kos_menu'),
    url(r'^umk/kos/edit_zachot/(?P<id>(\d+))/$',views.edit_kos,{'title':'Перечень вопросов к зачету','kos_id':'voprosy_k_zachoty'},name='kos_vorosy_zachot'),
    url(r'^umk/kos/edit_exam/(?P<id>(\d+))/$',views.edit_kos,{'title':'Перечень вопросов к экзамену','kos_id':'voprosy_k_exameny'},name='kos_vorosy_exam'),
    url(r'^umk/kos/edit_cur_control/(?P<id>(\d+))/$',views.edit_kos,{'title':'Типовые задания для текущего контроля','kos_id':'tekushii_kontrol'},name='kos_cur_control'),
    url(r'^umk/kos/edit_labs/(?P<id>(\d+))/$',views.edit_kos,{'title':'Комплект заданий для лабораторной работы','kos_id':'zad_lab_rab'},name='kos_labs'),
    url(r'^umk/kos/edit_prakt/(?P<id>(\d+))/$',views.edit_kos,{'title':'Комплект заданий для практической работы','kos_id':'zad_prakt_rab'},name='kos_prakt'),
    url(r'^umk/kos/edit_kontr_work/(?P<id>(\d+))/$',views.edit_kos,{'title':'Комплект заданий для контрольной работы','kos_id':'zad_konr_rabot'},name='kos_kontr_work'),
    url(r'^umk/kos/edit_referat/(?P<id>(\d+))/$',views.edit_kos,{'title':'Темы рефератов (эссе, докладов, сообщений)','kos_id':'themes_referat'},name='kos_referat'),
    url(r'^umk/kos/edit_zadachi/(?P<id>(\d+))/$',views.edit_kos,{'title':'Комплект разноуровневых заданий (задач)','kos_id':'reshenie_zadach'},name='kos_zadachi'),
    url(r'^zapolnenie/$', views.zapolnenie),
    url(r'^$', views.index, name = 'index.html'),
]